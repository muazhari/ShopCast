...
class LiveStreamScreen extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      // pre-popped out message in chat feature when in stream screen.
      listMessages: [
        // {
        //   userId: 'user1',
  ...
  onPressSend = () => {
    const { message, listMessages, productId, productImageUrl, productUrl } = this.state

    // if user insert product to chat feature.
    if (productId !== null && productImageUrl !== null && productUrl !== null) {
      this.setState({ message: '' })
      Keyboard.dismiss()

      // copy pre-listMessages to a new array and push credential state
      ...
      // update State after pushing chats with its credential.
      this.setState({
      ...
      // if user NOT insert product to chat feature, directly process message state.
    } else if (message !== '') {
      this.setState({ message: '' })
      Keyboard.dismiss()
  ...
  onPressCancelStreamer = () => {
    ...
    if (liveStatus === LiveStatus.REGISTER || liveStatus === LiveStatus.ON_LIVE) {
      return Alert.alert(
        'Alert',
  ...
  renderLiveText = () => {
    ...
    return (
      <View
        style={liveStatus === LiveStatus.ON_LIVE ? styles.wrapLiveText : styles.wrapNotLiveText}>
        <Text style={liveStatus === LiveStatus.ON_LIVE ? styles.liveText : styles.notLiveText}>
          LIVE
        </Text>
      </View>
    )
  ...
  onFinishDragProduct1 = () => {
    this.setState({
      message: 'Link for info about product 1',
      productId: 1,
      productImageUrl: 'https://cf.shopee.vn/file/3c18ee889c242196030a86b7ce86a59e_tn',
      productUrl:
  ...
  onFinishDragProduct2 = () => {
    this.setState({
      ...
      productId: 2,
      productImageUrl: 'https://cf.shopee.vn/file/1366956e12b7c40936a1e11ffe1bd486_tn',
  ...
  onFinishDragProduct3 = () => {
    this.setState({
      ...
      productId: 3,
      productImageUrl: 'https://cf.shopee.vn/file/31df73f75132ec3f979c39c550e249b5',
      productUrl:
        ...
        <ScrollView
          ...
          {listMessages.length > 0 &&
            listMessages.map((item, index) => {
              const { productId, productUrl, productImageUrl, userId, message } = item
              return (
                  ...
                  <View style={styles.messageItem}>
                    {!Utils.isNullOrUndefined(productId) &&
                      ...
                      !Utils.isNullOrUndefined(productImageUrl) && (
                        <TouchableWithoutFeedback onPress={() => this.onPressProduct(item)}>
                          <View style={styles.wrapSeeDetail}>
                            <Image source={{ uri: productImageUrl }} style={styles.iconProduct} />
                            <Text style={styles.textShowDetail}>Click here to see detail</Text>
                          </View>
                        </TouchableWithoutFeedback>
                      )}
  ...
  renderStreamerUI = () => {
    ...
    return (
      <View style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor="#2229b3" />
        <NodeCameraView
        ...
        <TouchableWithoutFeedback
          ...
          <View style={styles.container}>
            <NodePlayerView
              ...
              inputUrl={`${Utils.getRtmpPath() +
                Utils.getRoomName()}/replayfor${Utils.getUserId()}`}
              // inputUrl={
